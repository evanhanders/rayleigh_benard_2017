...
def Rayleigh_Benard(Rayleigh=1e6, Prandtl=1, nz=64, nx=None, aspect=4, restart=None, data_dir='./', max_writes=20,\
    ...
    profiles.add_task("plane_avg(u)", name="w")
    ...
    scalar.add_task("0.5*vol_avg(plane_avg(u)**2)", name="KE_x0")
    scalar.add_task("0.5*vol_avg((u-plane_avg(u))**2)", name="KE_x1")
    scalar.add_task("0.5*vol_avg(plane_avg(w)**2)", name="KE_z0")
    scalar.add_task("0.5*vol_avg((w - plane_avg(w))**2)", name="KE_z1")
    ...
    scalar.add_task("vol_avg(sqrt((w_avg * dz(u_avg))**2))", name="P_ma")
    scalar.add_task("vol_avg(sqrt((plane_avg(u_1*dx(u_1) + w_1*dz(u_1)))**2))", name="P_fa")
    scalar.add_task("vol_avg(sqrt((R*dz(uz_avg))**2))", name="P_mv")
    scalar.add_task("vol_avg(plane_avg(u))", name="u_momentum")
